...
# Sariq rang va chiroyli gap bloklari uchun CSS
st.markdown("""
<style>
...
# Sariq rangni va gap bloklarini xavfsiz integratsiya qilish uchun CSS
st.markdown("""
<style>
    .highlight {
...
# =========================================================
    ...
    with tab1:
        q = st.text_input("So'z kiriting:", placeholder="Masalan: maktab, til...", key="input_um")
        if q.strip():
            word = q.strip()
...
# =========================================================
    ...
    with tab_p1:
        q_pub = st.text_input("Publististik korpusdan so'z kiriting:", placeholder="Masalan: matbuot, xalq...", key="input_pub")
        if q_pub.strip():
            word = q_pub.strip()
            pat = re.compile(rf"({re.escape(word)}[b-df-hj-np-rt-vxz'aouei‘g‘shch]*)", re.IGNORECASE)
            res = df_pub[df_pub['Gap'].apply(lambda x: bool(pat.search(x)))] if not df_pub.empty else pd.DataFrame()
